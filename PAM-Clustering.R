# Clustering ASD patients using PAM over logistic PCA
...
output_folder = "C:\\Users\\gabim\\Documents\\Master\\Stalicla\\Work\\Repos\\Clustering-PTV\\Part_III\\Results\\PAM\\"

patients_clust = "PTV-PAM-patients.csv"
PAM_clust = "PTV-PAM-patients.png"
...
PAM_clust = glue(output_folder, PAM_clust)
...
# Do not trust this blindly!
library(factoextra)
sil_clust <- fviz_nbclust(PCA, pam, method = "silhouette") + labs(subtitle = "Silhouette method")
...
# Do not trust this blindly!
elbow <- fviz_nbclust(PCA, pam, method = "wss") + geom_vline(xintercept = 2, linetype = 2) + labs(subtitle = "Elbow method")
...
# PAM 
PAM.applied <- pam(PCA, num_clusters)
PAM_plot <- fviz_cluster(PAM.applied, data = PCA, ellipse.type = "convex")

# Save plot
library(cowplot)
save_plot(PAM_clust, PAM_plot)

# Internal validation for K-means with Silhouete method >> good classification according to Silhouette
# Do not trust this blindly!
silPAM <- silhouette(PAM.applied$cluster, dist(PCA))
summary(silPAM)
sil_plot <- fviz_silhouette(silPAM)
...
# Export clusters
patients.clusters <- cbind(patients, cluster = PAM.applied$cluster)
